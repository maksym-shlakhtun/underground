...
void Undeground::addHumanToTrain(const std::string &  _humanName, const std::string & _stationName)
{
	...
	for (int i = 0; i<getBranchesCount(); i++)
	{
		if (m_Branches[i]->moveHumanFromStationToTrain(_humanName, _stationName))
			return;
	}
	throw std::logic_error("This operation is not possible");
}

void Undeground::addHumanToTrain(const std::string &  _humanName, const std::string & _stationName, int _carriageNumber)
{
	hasAddHuman(_humanName);
	for (int i = 0; i<getBranchesCount(); i++)
	{
		 if (m_Branches[i]->moveHumanFromStationToTrain(_humanName, _stationName, _carriageNumber))
		 return;
	}
		throw std::logic_error("This operation is not possible");
}
